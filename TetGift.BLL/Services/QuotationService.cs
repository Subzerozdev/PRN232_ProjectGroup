...
namespace TetGift.BLL.Services
{
    public class QuotationService : IQuotationService
    {
        ...
        // user accept => create order (service riêng)
        ...
        // user reject => back to staff (revision++)
        ...
        public async Task ProposeItemDiscountsAsync(int quotationId, StaffProposeItemDiscountRequest req)
        {
            ...
            foreach (var line in req.Lines)
            {
                ...
                // 1) quotation_item.price = line total BEFORE discount
                var originalLineTotal = unitPrice * qty;
                qi.Price = Math.Round(originalLineTotal, 2);
                qiRepo.Update(qi);

                // 2) quotation_fee.description = discount percent
                //    quotation_fee.price = line total AFTER discount
                ...
                if (fee == null)
                {
                    fee = new QuotationFee
                    {
                        ...
                        Issubtracted = 0, // not used in your new meaning
            ...
            // Update quotation total as sum of discounted lines
        ...
        private async Task<QuotationDetailDto> BuildQuotationDetailAsync(int quotationId)
        {
            ...
            // load fees (batch) - take latest fee per item
        ...
        {
            // staff xem mọi quotation (tuỳ bạn sau này check role)
            return await BuildQuotationDetailAsync(quotationId);
        }

        public async Task<QuotationDetailDto> GetAdminQuotationDetailAsync(int quotationId)
        {
            // admin xem mọi quotation (tuỳ bạn sau này check role)
            return await BuildQuotationDetailAsync(quotationId);
        }
